				      ...
				      PointerPoint^ point, int state )
{
	...
	tp->state = state;
	tp->x = roundi( point->Position.X );
	tp->y = roundi( point->Position.Y );
	return tp;
}
...
LCUIInputDriver::LCUIInputDriver()
{
...
void LCUIInputDriver::OnPointerPressed( CoreWindow^ sender,
					PointerEventArgs^ args )
{
	...
	pos.x = roundi( position.X );
	pos.y = roundi( position.Y );
	ev.button.x = pos.x;
	ev.button.y = pos.y;
...
void LCUIInputDriver::OnPointerMoved( CoreWindow^ sender, PointerEventArgs^ args )
{
	Point position;
	LCUI_Pos pos;
	LCUI_TouchPoint tp;
	...
	ev.motion.x = pos.x;
	ev.motion.y = pos.y;
	ev.motion.xrel = roundi( position.X - m_mouse.position.Y );
	ev.motion.yrel = roundi( position.Y - m_mouse.position.Y );
	m_mouse.position = position;
	LCUI_TriggerEvent( &ev, NULL );
	LCUI_DestroyEvent( &ev );
	LCUICursor_SetPos( pos );
...
void LCUIInputDriver::OnPointerReleased( CoreWindow^ sender, PointerEventArgs^ args )
{
	LCUI_Pos pos;
	LCUI_TouchPoint tp;
	LCUI_SysEventRec ev;
...
void LCUIInputDriver::OnPointerWheelChanged( CoreWindow^ sender, PointerEventArgs^ args )
{
	LCUI_SysEventRec ev;
	PointerPoint^ point = args->CurrentPoint;
	PointerPointProperties^ pointProps = point->Properties;
...
void LCUIInputDriver::OnKeyDown( CoreWindow^ sender, KeyEventArgs^ args )
{
	LCUI_SysEventRec ev;
...
void LCUIInputDriver::OnKeyUp( CoreWindow^ sender, KeyEventArgs^ args )
{
	LCUI_SysEventRec ev;
	ev.type = LCUI_KEYUP;
	ev.key.code = static_cast<int>(args->VirtualKey);
...
void LCUIInputDriver::RegisterIME( void )
{
...
void LCUIInputDriver::SelectIME( void )
{
